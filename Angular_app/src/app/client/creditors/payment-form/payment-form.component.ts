...
export class PaymentFormComponent {
  ...
  ngOnInit() {
    console.log(
      'the form id that i received is ' + PaymentFormComponent.formId
    );
    ...
    this.http
      ...
      .subscribe((response) => {
        ...
        for (let i = 0; i < fields.length; i++) {
          const id = parseInt(
            fields[i].getElementsByTagName('id')[0].textContent
          );
  ...
  updateFieldErrorMessage(fieldName: string) {
    ...
    if (control?.hasError('required')) {
      ...
      console.log(value);
      this.fields.find((field) => field.name === fieldName).errorMessage =
        'Please respect the valid format.';
    } else {
      this.fields.find((field) => field.name === fieldName).errorMessage = '';
    }
  }
  ...
  checkClient(clientPhone: string, creanceId: number) {
    ...
    this.http
      ...
      .subscribe((response) => {
        ...
        const xmlResponse = parser.parseFromString(response, 'text/xml');
        const clientExists =
          xmlResponse.getElementsByTagName('clientExists')[0].textContent;
        const clientHasCreance =
          xmlResponse.getElementsByTagName('clientHasCreance')[0].textContent;
        const message =
